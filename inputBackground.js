document.addEventListener('DOMContentLoaded', function () {
    const inputBackgroundBtn = document.getElementById('inputBackgroundBtn');
    ...
    let savedBackgroundColor = localStorage.getItem('inputTextareaBackgroundColor') || '#FFFFFF';
    inputTextArea.style.backgroundColor = savedBackgroundColor;

    inputBackgroundBtn.addEventListener('click', function () {
        // Blur the input text box area
        inputTextArea.style.filter = 'blur(5px)';

        // Create color picker modal
        const colorPickerModal = document.createElement('div');
        colorPickerModal.style.position = 'absolute'; // Positioned relative to the input text box
        ...
        // Position the modal at the center of the input text box area
        const inputRect = inputTextArea.getBoundingClientRect();
        colorPickerModal.style.top = `${inputRect.top + window.scrollY + inputRect.height / 2}px`;
        colorPickerModal.style.left = `${inputRect.left + window.scrollX + inputRect.width / 2}px`;
        ...
        okButton.addEventListener('click', function () {
            ...
            inputTextArea.style.backgroundColor = savedBackgroundColor;
            localStorage.setItem('inputTextareaBackgroundColor', savedBackgroundColor);
