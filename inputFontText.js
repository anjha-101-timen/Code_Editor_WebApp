document.addEventListener('DOMContentLoaded', function () {
    const inputFontBtn = document.getElementById('inputFontBtn');
    const inputTextArea = document.getElementById('input-area');

    // Retrieve saved font from local storage or default to Ubuntu Medium 500 Bold
    let savedFont = localStorage.getItem('inputTextareaFont') || "'Ubuntu', monospace";
    inputTextArea.style.fontFamily = savedFont;

    inputFontBtn.addEventListener('click', function () {
        // Blur only the input text box area
        inputTextArea.style.filter = 'blur(5px)';
        ...
        fontModal.style.position = 'absolute'; // Positioned relative to the input text box
        fontModal.style.backgroundColor = '#ffffff'; // Light background for better contrast
        fontModal.style.padding = '20px';
        fontModal.style.borderRadius = '8px';
        fontModal.style.boxShadow = '0 4px 10px rgba(0, 0, 0, 0.5)';
        fontModal.style.zIndex = '1003'; // Higher than other elements

        // Position the modal at the center of the input area
        const inputRect = inputTextArea.getBoundingClientRect();
        fontModal.style.top = `${inputRect.top + window.scrollY + inputRect.height / 2}px`;
        fontModal.style.left = `${inputRect.left + window.scrollX + inputRect.width / 2}px`;
        ...
        searchInput.style.width = 'calc(100% - 22px)'; // Adjust width for padding
        ...
        // Add select menu for fonts
        const fontSelect = document.createElement('select');
        fontSelect.style.width = '100%';
        fontSelect.style.padding = '10px';
        fontSelect.style.borderRadius = '4px';
        fontSelect.style.border = '1px solid #ccc';
        fontSelect.style.background = '#ffffff'; // White background for better visibility
        fontSelect.style.color = '#333'; // Dark text color for contrast
        fontSelect.style.fontSize = '1em';
        fontSelect.style.overflowY = 'auto'; // Enable vertical scroll
        fontSelect.style.maxHeight = '200px'; // Set max height to limit dropdown size
        ...
        function populateFontSelect(fonts) {
            ...
            fonts.forEach(font => {
                ...
                option.style.fontFamily = font.family; // Highlight the font in the option
                option.style.height = '30px'; // Fixed height for option
                option.style.lineHeight = '30px'; // Align text vertically
                fontSelect.appendChild(option);
            });
            fontSelect.value = savedFont; // Set selected after repopulating
        }

        // Initial population
        populateFontSelect(googleFonts.sort((a, b) => a.name.localeCompare(b.name)));

        // Set the selected font based on savedFont
        ...
        okButton.addEventListener('click', function () {
            ...
            inputTextArea.style.fontFamily = selectedFont;
            localStorage.setItem('inputTextareaFont', selectedFont);
            document.body.removeChild(modalOverlay); // Close modal
            inputTextArea.style.filter = 'none'; // Remove blur
        });
